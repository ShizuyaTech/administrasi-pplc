...
@section('title', 'Approval Overtime - Supervisor')
@section('page-title', 'Approval Overtime (Supervisor - Tahap 1)')
...
<div class="space-y-6" x-data="overtimeApproval()">
    ...
    @if(count($grouped) > 0)
        ...
        @foreach($grouped as $dateKey => $dateGroup)
            <div class="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
                ...
                <!-- Loop through Types (Harian, Susulan) -->
                @foreach($dateGroup['types'] as $typeKey => $typeGroup)
                    <div class="p-6 border-b last:border-b-0">
                        ...
                        <!-- Loop through Batches (per Leader/Foreman) -->
    ...
    @else
        ...
        <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-12 text-center">
            ...
            <p class="text-gray-500">Tidak ada overtime yang menunggu approval untuk periode yang dipilih.</p>
        </div>
    @endif
    ...
    <div x-show="showRejectModal" 
        ...
        <div class="bg-white rounded-lg shadow-xl max-w-md w-full mx-4 p-6" @click.stop>
            <h3 class="text-lg font-semibold text-gray-800 mb-4">Reject Overtime</h3>
            ...
            <textarea x-model="rejectionReason"
                      rows="4"
                      placeholder="Contoh: Data tidak lengkap, jam tidak sesuai, dll."
...
function overtimeApproval() {
    return {
        ...
        toggleSelectGroup(groupKey) {
            // Select all checkboxes in current date-type group
        ...
        async approveSelected() {
            ...
            if (!confirm(`Approve ${this.selectedCount} karyawan?`)) {
                return;
            }
            
            try {
                const response = await fetch('{{ route("overtimes.supervisor.bulk-approve") }}', {
                    method: 'POST',
                    headers: {
        ...
        async rejectSelected() {
            ...
            try {
                const response = await fetch('{{ route("overtimes.supervisor.bulk-reject") }}', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                        'X-CSRF-TOKEN': '{{ csrf_token() }}'

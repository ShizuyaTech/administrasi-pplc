...
@section('page-title', 'Manage Permissions untuk Role: ' . $role->name)
            ...
            <div class="flex justify-between items-center">
                <div>
                    ...
                    <p class="text-xs text-gray-500 mt-2">
                        Saat ini memiliki <span class="font-semibold">{{ $role->permissions->count() }}</span> permissions
                    </p>
                </div>
        ...
        <form action="{{ route('roles.permissions.update', $role) }}" method="POST">
                                ...
                                @foreach($permissions as $permission)
                                    <label class="flex items-start p-3 bg-gray-50 rounded-lg hover:bg-gray-100 cursor-pointer transition">
                                        ...
                                        <div class="ml-3 flex-1">
                                            ...
                                            <span class="text-xs text-gray-500 block mt-0.5">{{ $permission->slug }}</span>
                                            @if($permission->description)
                                                <span class="text-xs text-gray-400 block mt-1">{{ $permission->description }}</span>
                                            @endif
                                        </div>
                                    </label>
                                @endforeach
            ...
            @else
                <div class="text-center py-12">
                    ...
                    <p class="mt-2">
                        <a href="{{ route('permissions.create') }}" class="text-indigo-600 hover:text-indigo-800 text-sm font-medium">

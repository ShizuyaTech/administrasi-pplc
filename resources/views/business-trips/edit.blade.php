...
@section('title', 'Edit Surat Perjalanan Dinas')
@section('page-title', 'Edit Surat Perjalanan Dinas')

@section('content')
<div class="max-w-4xl mx-auto">
    <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
        <form method="POST" action="{{ route('business-trips.update', $businessTrip) }}" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                ...
                <div>
                    ...
                    <input type="text" id="letter_number" name="letter_number" value="{{ old('letter_number', $businessTrip->letter_number) }}" required class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 @error('letter_number') border-red-500 @enderror">
                ...
                <div>
                    ...
                    <select id="section_id" name="section_id" required class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 @error('section_id') border-red-500 @enderror">
                        ...
                        @foreach($sections as $section)
                            <option value="{{ $section->id }}" {{ old('section_id', $businessTrip->section_id) == $section->id ? 'selected' : '' }}>{{ $section->name }}</option>
                        @endforeach
                    </select>
            ...
            <div class="mb-6">
                ...
                <input type="text" id="employee_name" name="employee_name" value="{{ old('employee_name', $businessTrip->employee_name) }}" required class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 @error('employee_name') border-red-500 @enderror">
            ...
            <div class="mb-6">
                ...
                <input type="text" id="destination" name="destination" value="{{ old('destination', $businessTrip->destination) }}" required class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 @error('destination') border-red-500 @enderror">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                <div>
                    ...
                    <input type="date" id="departure_date" name="departure_date" value="{{ old('departure_date', $businessTrip->departure_date->format('Y-m-d')) }}" required class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 @error('departure_date') border-red-500 @enderror">
                ...
                <div>
                    ...
                    <input type="date" id="return_date" name="return_date" value="{{ old('return_date', $businessTrip->return_date->format('Y-m-d')) }}" required class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 @error('return_date') border-red-500 @enderror">
            ...
            <div class="mb-6">
                ...
                <textarea id="purpose" name="purpose" rows="4" required class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 @error('purpose') border-red-500 @enderror">{{ old('purpose', $businessTrip->purpose) }}</textarea>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                <div>
                    ...
                    <select id="transport" name="transport" required class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 @error('transport') border-red-500 @enderror">
                        ...
                        <option value="Mobil Dinas" {{ old('transport', $businessTrip->transport) == 'Mobil Dinas' ? 'selected' : '' }}>Mobil Dinas</option>
                        <option value="Mobil Pribadi" {{ old('transport', $businessTrip->transport) == 'Mobil Pribadi' ? 'selected' : '' }}>Mobil Pribadi</option>
                        <option value="Motor" {{ old('transport', $businessTrip->transport) == 'Motor' ? 'selected' : '' }}>Motor</option>
                        <option value="Pesawat" {{ old('transport', $businessTrip->transport) == 'Pesawat' ? 'selected' : '' }}>Pesawat</option>
                        <option value="Kereta" {{ old('transport', $businessTrip->transport) == 'Kereta' ? 'selected' : '' }}>Kereta</option>
                        <option value="Travel" {{ old('transport', $businessTrip->transport) == 'Travel' ? 'selected' : '' }}>Travel</option>
                    </select>
                ...
                <div>
                    ...
                    <input type="number" id="estimated_cost" name="estimated_cost" value="{{ old('estimated_cost', $businessTrip->estimated_cost) }}" min="0" step="1000" class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 @error('estimated_cost') border-red-500 @enderror">
            ...
            @php
                $existingCardUsage = $businessTrip->cardUsages->first();
            @endphp
            <div id="card-usage-section" class="mb-6 border border-indigo-200 rounded-lg p-6 bg-indigo-50" style="display: {{ $businessTrip->transport == 'Mobil Dinas' ? 'block' : 'none' }};">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                    ...
                    <div>
                        ...
                        <input type="text" id="card_initial_balance_display" readonly class="w-full px-4 py-3 border border-gray-300 rounded-lg bg-gray-100 text-gray-600" value="{{ $existingCardUsage ? 'Rp ' . number_format($existingCardUsage->initial_balance, 0, ',', '.') : '' }}">
                        <input type="hidden" id="card_initial_balance" name="card_initial_balance" value="{{ $existingCardUsage ? $existingCardUsage->initial_balance : '' }}">
                    </div>
                </div>
                
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                    <div>
                        <label for="card_usage_amount" class="block text-sm font-medium text-gray-700 mb-2">Jumlah Pemakaian (Rp) <span class="text-red-500">*</span></label>
                        <input type="number" id="card_usage_amount" name="card_usage_amount" min="0" step="1000" value="{{ old('card_usage_amount', $existingCardUsage ? $existingCardUsage->usage_amount : '') }}" class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500">
                        <p class="mt-1 text-xs text-gray-500">Total tol + parkir (aktual)</p>
                    </div>
                    
                    <div>
                        <label for="card_final_balance_display" class="block text-sm font-medium text-gray-700 mb-2">Saldo Akhir (Otomatis)</label>
                        <input type="text" id="card_final_balance_display" readonly class="w-full px-4 py-3 border border-gray-300 rounded-lg bg-gray-100 text-green-600 font-semibold" value="{{ $existingCardUsage ? 'Rp ' . number_format($existingCardUsage->final_balance, 0, ',', '.') : '' }}">
                    </div>
                </div>
                
                <div>
                    <label for="card_usage_notes" class="block text-sm font-medium text-gray-700 mb-2">Rincian Pemakaian</label>
                    <textarea id="card_usage_notes" name="card_usage_notes" rows="2" class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500" placeholder="Contoh: Tol Cikampek Rp 50.000, Parkir Mall Rp 10.000">{{ old('card_usage_notes', $existingCardUsage ? $existingCardUsage->usage_notes : '') }}</textarea>
                </div>

                @if($existingCardUsage)
                <div class="mt-4 p-4 bg-blue-50 border border-blue-200 rounded-lg">
                    <p class="text-sm text-blue-800">
                        <strong>Info:</strong> Kartu yang digunakan sebelumnya: <strong>{{ $existingCardUsage->card->card_number }}</strong> ({{ $existingCardUsage->card->card_type_name }})
                    </p>
                </div>
                @endif
            </div>
            ...
            <div class="mb-6">
                ...
                <select id="status" name="status" required class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 @error('status') border-red-500 @enderror">
                    <option value="draft" {{ old('status', $businessTrip->status) == 'draft' ? 'selected' : '' }}>Draft</option>
                    <option value="approved" {{ old('status', $businessTrip->status) == 'approved' ? 'selected' : '' }}>Approved</option>
                    <option value="completed" {{ old('status', $businessTrip->status) == 'completed' ? 'selected' : '' }}>Completed</option>
                    <option value="cancelled" {{ old('status', $businessTrip->status) == 'cancelled' ? 'selected' : '' }}>Cancelled</option>
                </select>
            ...
            <div class="mb-4">
                ...
                <div class="flex items-center space-x-3">
                    <a href="{{ Storage::url($businessTrip->attachment) }}" target="_blank" class="text-indigo-600 hover:text-indigo-800 underline">
                        {{ basename($businessTrip->attachment) }}
                    </a>
                    <span class="text-gray-500 text-sm">(klik untuk buka)</span>
                </div>
            </div>
            @endif
            ...
            <!-- New Attachment -->
            <div class="mb-6">
                <label for="attachment" class="block text-sm font-medium text-gray-700 mb-2">Ganti Lampiran (PDF/Image, max 2MB)</label>
                ...
                <p class="mt-1 text-sm text-gray-500">Kosongkan jika tidak ingin mengganti</p>
            </div>
            ...
            <div class="flex justify-end space-x-3">
                ...
                <button type="submit" class="px-6 py-3 bg-indigo-600 hover:bg-indigo-700 text-white font-medium rounded-lg">Update SPD</button>
            </div>
        </form>
    </div>
</div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (employeeNameInput) {
        initEmployeeAutocomplete(employeeNameInput, {
            sectionIdGetter: () => {
                ...
                return sectionSelect ? sectionSelect.value : '{{ $businessTrip->section_id }}';
            },
            onSelect: (employee) => {
    ...
    const existingCardId = '{{ $existingCardUsage ? $existingCardUsage->card_id : "" }}';
    ...
    function loadCards() {
        const sectionId = sectionSelect ? sectionSelect.value : '{{ $businessTrip->section_id }}';
            ...
            .then(cards => {
                ...
                if (cards.length === 0) {
                    ...
                    cards.forEach(card => {
                        ...
                        const isSelected = existingCardId && existingCardId == card.id ? 'selected' : '';
                        cardSelect.innerHTML += `<option value="${card.id}" data-balance="${card.current_balance}" ${isSelected}>${card.card_number} - ${cardTypeName} (${balance})</option>`;
                    });
                    
                    // If card was preselected, trigger change event
                    if (existingCardId) {
    ...
    if (transportSelect.value === 'Mobil Dinas') {
        loadCards();
        // Calculate initial final balance if values exist
        if (cardUsageAmount.value) {
            calculateFinalBalance();
        }
    }
});
